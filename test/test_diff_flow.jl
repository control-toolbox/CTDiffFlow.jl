...
λ = [1.,2]
t0 = 0. ; tf = 2.; x0 = [λ[2],1.,1]; 
sol_∂xO_flow = diagm([exp(tf*λ[1]),exp(tf*λ[2]),exp(tf*(λ[1]-λ[2]))])

∂x0_flow_var = DiffFlow.build_∂x0_flow_var(fun1,t0,x0,tf,λ)
@test isapprox(sol_∂xO_flow, ∂x0_flow_var(t0,x0,tf,λ;reltol=reltol, abstol=abstol) , atol = tol_error) 
...
@test isapprox(my_∂x0_flow(t0,x0,tf,λ;reltol=reltol, abstol=abstol), ∂x0_flow_var(t0,x0,tf,λ;reltol=reltol, abstol=abstol) , atol=tol_error) 

# Derivative with respect to λ
sol_∂λ_flow = [λ[2]*tf*exp(λ[1]*tf) exp(λ[1]*tf)
                     0            tf*exp(λ[2]*tf)
            tf*exp((λ[1]-λ[2])*tf) -tf*exp((λ[1]-λ[2])*tf)]

funx0(λ) = [λ[2],1.,1]           
∂λ_flow_var = DiffFlow.build_∂λ_flow_var(fun1,t0,funx0,tf,λ)
λ0 = zeros(3,2); λ0[1,2] = 1.
@test isapprox(sol_∂λ_flow, ∂λ_flow_var(t0,funx0,tf,λ; reltol=reltol, abstol=abstol), atol=tol_error)
...
println("ddd" , (my_∂x0_flow(t0,x0,tf,λ;reltol=reltol, abstol=abstol)-∂x0_flow(t0,x0,tf,λ;reltol=reltol, abstol=abstol)))
@test isapprox(sol_∂xO_flow, ∂x0_flow(t0,x0,tf,λ;reltol=reltol, abstol=abstol) , atol = tol_error) 
@test isapprox(my_∂x0_flow(t0,x0,tf,λ;reltol=reltol, abstol=abstol), ∂x0_flow(t0,x0,tf,λ;reltol=reltol, abstol=abstol) , atol=tol_error) 
...
println(sol_∂λ_flow)
@test isapprox(sol_∂λ_flow, ∂λ_flow(t0,funx0,tf,λ; reltol=reltol, abstol=abstol), atol=tol_error)
