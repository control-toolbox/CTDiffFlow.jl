...
x0=[1.5,3];
par = [3.]
T,X,haccept,Trej,hrej,Ttot,err_loc_est,err_loc_exact,err_abs = myode43(bruss,x0,par,[t0, tf],Rtol,Atol)
...
scatter!(p3,Ttot[2:end],err_loc_est,yscale = :log10,c=:blue,labels=false,
markersize = 3, markershape = :x)
plot!(p3,[t0, tf],[Atol, Atol],c=:black,labels=false)

plot!(p3,Ttot[2:end],err_loc_exact,c=:green,yscale = :log10,labels="local estimated errors")
scatter!(p3,Ttot[2:end],err_loc_exact,c=:green,yscale = :log10,labels=false,
markersize = 3,markershape = :x)
plot!(p3,Ttot[2:end],err_abs,c=:black,yscale = :log10,labels="global exact errors")
scatter!(p3,Ttot[2:end],err_abs,c=:black,yscale = :log10,labels=false,markershape = :x,
markersize = 3)
