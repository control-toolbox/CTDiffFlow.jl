...
function my_∂λ_flow(tspan, xX0, λ; ode_kwargs...)
    #n,p = size(xX0)
    fun2(xX, λ, t) = [
        fun1(xX[:, 1], λ, t) fun1(xX[:, 2:end], λ, t)+[
            xX[1, 1] 0;
            0 xX[2, 1];
            xX[3, 1] xX[3, 1]
        ]
...
sol_var = ∂λ_flow_var(
    (t0, tf), funx0, λ; reltol=reltol, abstol=abstol, internalnorm=(u, t)->norm(u[:, 1:1])
)
...
sol_diff_auto_flow_tf, T = ∂λ_flow(
    t0, funx0, tf, λ; reltol=reltol, abstol=abstol, print_times=true
) #, alg = Euler(),adaptive=false, dt = (tf-t0)/N,print_times=true)
